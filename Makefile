...
TEST_VALIDATOR_DIR := $(TEST_DIR)/validators
...
ANALYSIS_SCRIPTS_DIR := $(ANALYSIS_DIR)/scripts
ANALYSIS_REL_LATENCY_FILE := $(ANALYSIS_DIR)/system/rel_lat.txt

PYTHON_EXEC := $(ANALYSIS_SCRIPTS_DIR)/.env/bin/python3
...
run: $(EXECUTABLE)
	...
	$(PYTHON_EXEC) $(TEST_VALIDATOR_DIR)/validate_offsets.py $(EXAMPLE_OUTPUT_FILE)
	$(PYTHON_EXEC) $(ANALYSIS_SCRIPTS_DIR)/generate_gantt.py $(EXAMPLE_OUTPUT_FILE) $(EXAMPLE_GANTT_FILE)
	$(PYTHON_EXEC) $(ANALYSIS_SCRIPTS_DIR)/generate_profile.py $(EXAMPLE_OUTPUT_FILE) $(EXAMPLE_REL_LATENCY_FILE)
...
$(TEST_CASES): %: $(EXECUTABLE)
	...
	@for config_file in $$(ls $(TEST_CONFIG_DIR)/$@/*.json 2>/dev/null); do \
		...
		EXEC_STATUS=$$?; \
		$(PYTHON_EXEC) $(TEST_VALIDATOR_DIR)/validate_offsets.py "$$OUTPUT_FILE" >> "$$LOG_FILE" 2>&1; \
		VAL_OFFSETS_STATUS=$$?; \
		$(PYTHON_EXEC) $(TEST_VALIDATOR_DIR)/validate_output.py --check-order exec_name_total_offsets \
...
analyze:
	...
	@for out_dir in $$(ls $(ANALYSIS_OUTPUT_DIR) 2>/dev/null); do \
		...
		LOG_NAME=$${LOG_DIR}/summary.log; \
		$(PYTHON_EXEC) $(ANALYSIS_SCRIPTS_DIR)/generate_summary_plot_v1.py \
			"$$AGGREG_DIR" \
			"$$PLOT_NAME" \
			>> "$$LOG_NAME" 2>&1; \
		PLOT_STATUS=$$?; \
