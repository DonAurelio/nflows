...
def compute_fig_size(yaml_data, time_unit, use_numa):
    ...
    fig_height = max(6, min(12, (num_resources + num_comms) / 2))  # Adjusted height scaling
...
def plot_gantt(yaml_data, output_file, time_unit='us', payload_unit='B', use_numa=True, title=None, xlabel=None, ylabel=None, resource_label=None):
    ...
    # Adjust spacing to prevent bars from shrinking too much
    min_bar_height = 0.4  # Ensure readability
    y_spacing = max(1.0, min(2.0, 10 / len(resource_map)))  # Adaptive spacing
    ...
    for task, times in yaml_data['exec_name_compute_offsets'].items():
        ...
        y_position = resource_map[resource_id] * y_spacing
    ...
    # Plot communications
    comm_offset_map = {r: 0 for r in resource_map}  # Track offsets to prevent overlap
    ...
    for comm, times in yaml_data['comm_name_write_offsets'].items():
        ...
        y_position = resource_map[resource_id] * y_spacing + comm_offset_map[resource_id] * min_bar_height

        comm_offset_map[resource_id] += 1  # Avoid overlap
        start = scale_time(times['start'], time_unit)
        end = scale_time(times['end'], time_unit)
    ...
    # Set y-axis labels
    ax.set_yticks([resource_map[r] * y_spacing for r in resource_ids])
