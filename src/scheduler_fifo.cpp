...
std::tuple<int, double> FIFO_Scheduler::get_best_core_id(const simgrid_exec_t *exec)
{
    ...
    name_to_time_range_payload_t comm_name_to_time_range_payload = common_filter_name_ts_range_payload(
    ...
    for (const auto& [key, _] : numa_id_to_payload) ordered_numa_ids.push_back(key);

    // Sort keys based on their corresponding values in the map
    std::sort(ordered_numa_ids.begin(), ordered_numa_ids.end(), [numa_id_to_payload](int a, int b) { 
        return numa_id_to_payload.at(a) < numa_id_to_payload.at(b); 
    });
...
std::tuple<simgrid_exec_t *, int, double> FIFO_Scheduler::next()
{
    ...
    std::unordered_map<std::string, uint64_t> name_to_data_locality_scores = this->get_data_locality_scores(ready_execs);

    std::sort(ready_execs.begin(), ready_execs.end(), [&](simgrid_exec_t *a, simgrid_exec_t *b) {
        return name_to_data_locality_scores[a->get_name()] > name_to_data_locality_scores[b->get_name()]; // Higher score first
    });
