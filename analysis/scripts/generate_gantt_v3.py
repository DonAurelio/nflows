...
def plot_gantt(yaml_data, output_file, time_unit='us', payload_unit='B', use_numa=True, title=None, xlabel=None, ylabel=None, resource_label=None):
    ...
    for comm, times in yaml_data['comm_name_read_offsets'].items():
        _, task_dest = comm.split('->')
        locality = yaml_data['name_to_thread_locality'][task_dest]
        resource_id = locality['numa_id'] if use_numa else locality['core_id']
        y_position = resource_map[resource_id] * offset - 0.5
        start = scale_time(times['start'], time_unit)
        end = scale_time(times['end'], time_unit)
        ax.plot([start, end], [y_position, y_position], color='red', linestyle='-', linewidth=2, label='Communication' if comm == list(yaml_data['comm_name_read_offsets'].keys())[0] else "")

    for comm, times in yaml_data['comm_name_write_offsets'].items():
        task_src, _ = comm.split('->')
        locality = yaml_data['name_to_thread_locality'][task_src]
        resource_id = locality['numa_id'] if use_numa else locality['core_id']
        y_position = resource_map[resource_id] * offset + 0.5
        start = scale_time(times['start'], time_unit)
        end = scale_time(times['end'], time_unit)
        ax.plot([start, end], [y_position, y_position], color='blue', linestyle='--', linewidth=2, label='Communication' if comm == list(yaml_data['comm_name_write_offsets'].keys())[0] else "")
